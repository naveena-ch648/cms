...
export default function WorkspacePage() {
  ...
  const handleFilesSelected = useCallback(async (selectedFiles: File[]) => {
    ...
    for (const file of selectedFiles) {
      ...
      const progress: UploadProgress = {
        fileId: uploadId, fileName: file.name, totalBytes: file.size,
        uploadedBytes: 0, percentage: 0, status: 'uploading', startedAt: Date.now(),
      };
      setUploads(prev => [...prev, progress]);

      filesApi.uploadFile(file, selectedFolderId, {}, (pct) => {
        setUploads(prev => prev.map(u => u.fileId === uploadId ? { ...u, percentage: pct, uploadedBytes: Math.round(file.size * pct / 100) } : u));
      }).then(() => {
        setUploads(prev => prev.map(u => u.fileId === uploadId ? { ...u, status: 'completed', percentage: 100 } : u));
        if (selectedFolderId) loadFiles(selectedFolderId);
      }).catch((err) => {
        setUploads(prev => prev.map(u => u.fileId === uploadId ? { ...u, status: 'failed', error: err.message || 'Upload failed' } : u));
      });
    }
  ...
  useEffect(() => {
    if (!workspaceId) return;
    workspacesApi.get(workspaceId).then(res => setWorkspaceName(res.data.data.name)).catch(() => {});
    loadFolders();
    loadFavorites();
    loadRecents();
  ...
  const handleSelectFolder = useCallback(async (folderId: string) => {
    ...
    try {
      ...
      loadFiles(folderId);
      // Record visit
      foldersApi.recordVisit(workspaceId, folderId).then(() => loadRecents()).catch(() => {});
    } catch {
      setError('Failed to load folder');
    }
  }, [workspaceId, loadRecents, loadFiles]);

  const handleNavigateBreadcrumb = useCallback((folderId: string | null) => {
    if (folderId) {
  ...
  const handleRenameFolder = useCallback(async (folderId: string, currentName: string) => {
    ...
    try {
      await foldersApi.update(workspaceId, folderId, { name: newName.trim() });
      await loadFolders();
  ...
  const handleToggleFavorite = useCallback(async (folderId: string, isFavorite: boolean) => {
    if (!workspaceId) return;
    try {
      if (isFavorite) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ borderBottom: '1px solid #e0e0e0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Breadcrumbs
            items={breadcrumbs}
        ...
        <div style={{ flex: 1, padding: 16, overflowY: 'auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h2 style={{ margin: 0 }}>
              {currentFolder ? currentFolder.name : workspaceName || 'Workspace'}
            </h2>
          ...
          {currentChildren.length === 0 && files.length === 0 && !selectedFolderId ? (
