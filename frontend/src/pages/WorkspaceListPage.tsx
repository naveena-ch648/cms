...
export default function WorkspaceListPage() {
  ...
  const { user } = useAuth();
  const isOrgAdmin = user?.organizationRole?.toLowerCase() === "admin";
  const canManageWorkspace =
    isOrgAdmin || user?.organizationRole?.toLowerCase() === "editor";
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1000, margin: "0 auto" }}>
      ...
      >
        <h1
          style={{ margin: 0, fontSize: 24, fontWeight: 700, color: "#1e293b" }}
        >
          Workspaces
        </h1>
        {canManageWorkspace && (
